...
public class MessageFilterParser implements FilterParser<MessageFilter, Message> {
    ...
    public Checker<Message> createChecker(MessageFilter annotation, Finder finder) {
        ...
        if (replyToMessage.value().isActive()) {
            unionExtractChecker.add(msg -> Optional.ofNullable(msg.getReplyToMessage()), finder.find(replyToMessage));
        }
        ...
        if (photo.value().isActive()) {
            unionExtractChecker.add(msg -> Optional.ofNullable(msg.getPhoto()), finder.find(photo));
        }
        ...
        if (entities.value().isActive()) {
            unionExtractChecker.add(msg -> Optional.ofNullable(msg.getEntities()), finder.find(entities));
        }
        MessageEntityArrayFilter captionEntites = annotation.captionEntityArray();
        if (captionEntites.value().isActive()) {
            unionExtractChecker.add(msg -> Optional.ofNullable(msg.getCaptionEntities()), finder.find(captionEntites));
        }
        ...
        if (pinnedMessage.value().isActive()) {
            unionExtractChecker.add(msg -> Optional.ofNullable(msg.getPinnedMessage()), finder.find(pinnedMessage));
        }
