...
class StringValidator(startWith: Array<String>,
                      ...
                      notContains: Array<String>) : Validator<String> {
    ...
    init {
        ...
        if (startWith.isNotEmpty()) {
            matchItems.add(ValidateItem(startWith, { obj, prefix -> obj.startsWith(prefix) }))
        }
        if (equalsWith.isNotEmpty()) {
            matchItems.add(ValidateItem(equalsWith, { obj, anObject -> obj == anObject }))
        }
        if (endWith.isNotEmpty()) {
            matchItems.add(ValidateItem(endWith, { obj, suffix -> obj.endsWith(suffix) }))
        }
        if (contains.isNotEmpty()) {
            matchItems.add(ValidateItem(contains, { obj, s -> obj.contains(s) }))
        }

        if (notContains.isNotEmpty()) {
            nonMatchItems.add(ValidateItem(notContains, { obj, s -> obj.contains(s) }))
        }
        if (notEndWith.isNotEmpty()) {
            nonMatchItems.add(ValidateItem(notEndWith, { obj, suffix -> obj.endsWith(suffix) }))
        }
        if (notStartWith.isNotEmpty()) {
            nonMatchItems.add(ValidateItem(notStartWith, { obj, prefix -> obj.startsWith(prefix) }))
        }
    }
