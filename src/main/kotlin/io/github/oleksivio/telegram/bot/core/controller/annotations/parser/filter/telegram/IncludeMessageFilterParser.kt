...
import io.github.oleksivio.telegram.bot.api.annotations.filter.composite.IncludeMessageFilter
...
@Component
class IncludeMessageFilterParser : FilterParser<IncludeMessageFilter, Message> {

    override val parserAnnotationClass = IncludeMessageFilter::class

    override fun createChecker(annotation: IncludeMessageFilter, finder: Finder): Validator<Message> {
        val unionExtractValidator = UnionExtractValidator<Message>()
        ...
        val photoArray = annotation.photoArray
        if (photoArray.status.isActive) {
            unionExtractValidator.add({ it.photo }, finder.find(photoArray))
        }
        ...
        val entityArray = annotation.entityArray
        if (entityArray.status.isActive) {
            unionExtractValidator.add({ it.entities }, finder.find(entityArray))
        }
        val captionEntityArray = annotation.captionEntityArray
        if (captionEntityArray.status.isActive) {
            unionExtractValidator.add({ it.captionEntities }, finder.find(captionEntityArray))
        }
